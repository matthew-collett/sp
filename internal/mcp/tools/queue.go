...
func RegisterQueue(s *mcp.Server, f *factory.Factory) {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "add_to_queue", Description: "Add a track to the playback queue by Spotify URI (e.g. spotify:track:4iV5W9uYEdYUVa79Axb7Rh)"}, t.addToQueue)
...
func (t *queue) addToQueue(ctx context.Context, _ *mcp.CallToolRequest, in addToQueueInput) (*mcp.CallToolResult, any, error) {
	...
	if err := sc.AddToQueue(ctx, in.URI); err != nil {
		return nil, nil, err
	}
	return &mcp.CallToolResult{Content: []mcp.Content{&mcp.TextContent{Text: fmt.Sprintf("Added %s to queue", in.URI)}}}, nil, nil
}
